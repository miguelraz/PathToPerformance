...
/* ------------------------------------------------------------------
   ...
   other authors for sake of parallelization.
...
#if BZLIB_TBB
static int numThreads=tbb::task_scheduler_init::default_num_threads();
#endif

static void compressStream(FILE *stream, FILE *zStream) {
    int error = BZ_STREAM_ERROR;

    // to read/write stdin/stdout in binary mode
    SET_BINARY_MODE(stream);
    SET_BINARY_MODE(zStream);

    if (error == BZ_STREAM_ERROR)
        error = BZ2_compressFile(stream, zStream, blockSize100k, verbosity, workFactor
#if BZLIB_TBB
                                ,3*numThreads
#endif
                                );
...
void compress ( Char *name )
{
...
#if BZLIB_CILK
   // User sets CILK_NWORKERS to the number of workers.
#elif BZLIB_TBB
   if( char *nthreads = getenv("TBB_NWORKERS") ) 
   ...
   fprintf(stderr, "compression time: %f\n", (end-start));
   fflush(stderr);
