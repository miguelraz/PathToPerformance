...
static void bsFinishWriteTBB(EState* s) {
    while (s->bsLive > 7) {
        s->zbits[s->numZ] = (UChar)(s->bsBuff >> 24);
        s->numZ++;
        s->bsBuff <<= 8;
...
int BZ_API(BZ2_compressFile)(FILE *stream, FILE *zStream, int blockSize100k, int verbosity, int workFactor, int ntoken) throw() {
    ...
    tbb::parallel_pipeline( 
	    ntoken,
	    tbb::make_filter<void,EState*>( tbb::filter::serial_in_order, [&]( tbb::flow_control& fc ) -> EState* {
	    ...
	    tbb::make_filter<EState*,EState*>( tbb::filter::parallel, []( EState*s ) -> EState* {
            if( s->nblock )
	        CompressOneBlock(s);
	    return s;
	    } ) &
	    tbb::make_filter<EState*,void>( tbb::filter::serial_in_order, [&]( EState* s ) {
	        if( s->nblock )
