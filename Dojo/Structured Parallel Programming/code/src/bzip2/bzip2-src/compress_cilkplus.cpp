...
/* ------------------------------------------------------------------
   ...
   THIS IS NOT THE ORIGINAL SOFTWARE.  It has been altered by
   others for sake of parallelization.
...
#include "cilk/cilk.h"
#include "../../pipeline/cilkplus/reducer_consume.h"
#endif        
...
*****************************************************************/
#if BZLIB_CILKPLUS
static void CompressOneBlock(EState *s);
...
static EState *BZ2_bzCompressInit
                    (int        blockSize100k,
                     int        verbosity,
                     int        workFactor)
{
...
// for writing un-aligned bit blocks to the output files
extern "C++"
struct bitblock {
        unsigned char bits, numbits;
        bitblock(unsigned char bits, int numbits) : bits(bits), numbits(numbits) {
...
static unsigned char copy_shift(unsigned char *out, bitblock carry_in, const unsigned char *in, unsigned int bytes)
{
...
static void SecondStage( EState* s, reducer_consume<OutputState,EState*>& sink ) {
    if( s->nblock )
        CompressOneBlock(s);
    sink.consume( s );
}
 
static void ThirdStage( OutputState* out_state, EState* s ) {
    if( s->nblock )
        out_state->putOneBlock(s); 
    FreeEState(s); 
} 

int BZ_API(BZ2_compressFile)(FILE *stream, FILE *zStream, int blockSize100k, int verbosity, int workFactor) throw() {
    ...
    // prologue (writing header to the output file)
    unsigned char header[4] = {BZ_HDR_B, BZ_HDR_Z, BZ_HDR_h, BZ_HDR_0 + blockSize100k };
    fwrite(header, 1, 4, zStream);

    // **************************************************************
    InputState in_state;
    OutputState out_state( zStream );
    reducer_consume<OutputState,EState*> sink(&out_state, ThirdStage);
    while( !feof(stream) && !ferror(stream) ) {
        EState *s = BZ2_bzCompressInit(blockSize100k, verbosity, workFactor);
        in_state.getOneBlock(stream,s); 
        cilk_spawn SecondStage(s, sink);
    };
    cilk_sync;
    // ***************************************************************
    
    // epilogue (write trailer to the output file) 
    uint32_t crc = out_state.combinedCRC;
    ...
    // writes the last CRC and flush
    out_state.write(trailer, 10, bitblock());
    out_state.flush();

    if (verbosity)
        report(in_state.nbytes_in, (out_state.total + 7)/8);


    fflush(zStream);
    return ferror(zStream) ? BZ_IO_ERROR : BZ_OK;
}
...
static void CompressOneBlock(EState *s)
{
    ...
    if (s->verbosity >= 2)
        VPrintf3("    block crc = 0x%08x, combined = 0x%08x, size = %d\n", s->blockCRC, s->combinedCRC, s->nblock);
